...
test.describe("Yaksha", () => {
   ...
   */
  test("TC1 - Verify My Info tab Loads", async ({ page }) => {
   ...
   */
  test("TC2 - Verify required field error on blank name", async ({ page }) => {
    await myinfoPage.clearAndEnterName();
    const nameValue = await page.locator("//span[text()='Required']").allInnerTexts();
    expect(nameValue.length).toBeGreaterThan(0);
    expect(nameValue).toContain("Required");
  });
 ...
 */
  test("TC3 - Verify name gets edited", async ({ page }) => {
    const newName = `TestUser_${Math.floor(Math.random() * 10000)}`;
    await myinfoPage.updateUniqueNAmeAndVerifyName(newName);
    const updatedName = await page.locator("//p[@class='oxd-userdropdown-name']").innerText();
 ...
 */
  ...
  test("TC9 - Upload dependent attachment", async ({ page }) => {
    ...
    await dependentsPage.UploadAttachmentInDependent(comnt);
